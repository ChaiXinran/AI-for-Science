...
init_server_defaults "pwv_v1_3h"
...
run_python_module nowcasting.cli.pwv.v1.train \
    --data_root "${PRECIP_ROOT}" \
    --pwv_root "${PWV_ROOT}" \
    --save_dir "${RUN_ROOT}/checkpoints/pwv_v1_3h" \
    --readme_ckpt "${RUN_ROOT}/checkpoints/pwv_v1_3h_model.ckpt" \
    --device "${DEVICE}" \
    --input_length 9 \
    --total_length 39 \
    --img_height 96 \
    ...
    --grad_clip 1.0 \
    --log_interval 100 \
    2>&1 | tee "${RUN_ROOT}/logs/train_pwv_v1_3h.log"
