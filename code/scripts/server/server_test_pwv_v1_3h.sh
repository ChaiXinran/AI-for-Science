...
run_python_module nowcasting.cli.pwv.v1.test \
    --data_root "${PRECIP_ROOT}" \
    --pwv_root "${PWV_ROOT}" \
    --checkpoint "${RUN_ROOT}/checkpoints/pwv_v1_3h_model.ckpt" \
    --output_dir "${RUN_ROOT}/results/pwv_v1_3h" \
    --device "${DEVICE}" \
    --split test \
    --input_length 9 \
    --total_length 39 \
    ...
    2>&1 | tee "${RUN_ROOT}/logs/test_pwv_v1_3h.log"
