...
python -u code/test/radar.py \
  ...
  --split_manifest "${SPLIT_MANIFEST}" --require_contiguous --split test \
  --max_samples "${MAX_TEST_SAMPLES}" --max_samples_strategy uniform \
  --num_save_samples 10 --output_dir "${SEED_ROOT}/results/radar" \
  --device "${DEVICE}" --input_length 9 --total_length 39 \
...
run_head() {
  ...
  if [[ "${RETEST_ONLY}" != "1" ]]; then
    ...
    [[ -f "${SEED_ROOT}/checkpoints/${name}/best_state_dict.ckpt" ]] || {
      echo "Missing checkpoint for retest: ${name}"; exit 1;
    }
  fi
