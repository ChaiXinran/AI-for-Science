...
from nowcasting.cli.custom.train_adversarial import (
    append_epoch_log,
    autocast_context,
...
def build_parser():
    parser = argparse.ArgumentParser(description="Train PWV-coupled NowcastNet")
    ...
    parser.add_argument("--save_dir", type=str, default="../checkpoints/pwv_coupled")
    parser.add_argument("--readme_ckpt", type=str, default="../checkpoints/pwv_coupled_model.ckpt")
...
def train_one_epoch(generator, discriminator, loader, opt_g, opt_d, scaler_g, scaler_d, args):
    ...
    for step, batch in enumerate(loader, 1):
        ...
        if step % args.log_interval == 0:
            ...
            for key in ("g_total", "d_total", "forecast", "evolution", "c_mean"):
...
def validate(generator, loader, args):
    ...
    for batch in loader:
        ...
        seen += frames.size(0)
    return total / max(seen, 1), c_mean / max(seen, 1)
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        val_loss, val_c_mean = validate(generator, val_loader, args)
