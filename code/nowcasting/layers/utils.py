...
class spectral_norm(nn.Module):
    ...
    def _update_u_v(self):
        ...
        device_type = w.device.type if w.device.type in ("cuda", "cpu") else "cuda"
        autocast_ctx = torch.amp.autocast(device_type=device_type, enabled=False) if hasattr(torch, "amp") else nullcontext()
        ...
        with autocast_ctx:
            ...
            for _ in range(self.power_iterations):
                v_f = l2normalize(torch.mv(torch.t(w_mat_f), u_f))
                u_f = l2normalize(torch.mv(w_mat_f, v_f))

            sigma = u_f.dot(w_mat_f.mv(v_f)).to(w.dtype)
            u.data.copy_(u_f.to(u.dtype))
            v.data.copy_(v_f.to(v.dtype))
