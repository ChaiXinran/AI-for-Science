...
EXPERIMENTS = {
    ...
    "PWV": {
        "metrics": "results/pwv_3h/metrics.json",
        "sample": "results/pwv_3h/sample_0000",
        "required": False,
    },
    "PWV Object": {
        "metrics": "results/pwv_object_3h/metrics.json",
        "sample": "results/pwv_object_3h/sample_0000",
        "required": False,
    },
}
...
def save_sample_grid(run_root, out_dir):
    ...
    for name, path in existing.items():
        ...
        if (path / "oc_00.png").exists():
            rows.append((f"Object center {name}", lambda i, p=path: colorize_gray(p / f"oc_{i:02d}.png", cmap="magma", stretch=True)))
        if (path / "om_00.png").exists():
            rows.append((f"Object mask {name}", lambda i, p=path: colorize_gray(p / f"om_{i:02d}.png", cmap="magma", stretch=True)))
    pwv_sample = existing.get("PWV") or existing.get("PWV Object")
...
def summarize(metrics, out_dir):
    ...
    summary = {
        "overall": {
            ...
            "pwv": metrics.get("PWV", {}).get("model"),
            "pwv_object": metrics.get("PWV Object", {}).get("model"),
            "pwv_v3_coupling_mean": metrics.get("PWV", {}).get("coupling_mean"),
            "pwv_v3_support_mean": metrics.get("PWV", {}).get("support_mean"),
        },
        "horizon_metrics": {
            "persistence": ref["horizon_metrics"]["persistence"],
            "radar_only": metrics.get("Radar-only", {}).get("horizon_metrics", {}).get("model"),
            "pwv": metrics.get("PWV", {}).get("horizon_metrics", {}).get("model"),
            "pwv_object": metrics.get("PWV Object", {}).get("horizon_metrics", {}).get("model"),
        },
        "event_metrics": {
            "persistence": ref["event_metrics"]["persistence"],
            "radar_only": metrics.get("Radar-only", {}).get("event_metrics", {}).get("model"),
            "pwv": metrics.get("PWV", {}).get("event_metrics", {}).get("model"),
            "pwv_object": metrics.get("PWV Object", {}).get("event_metrics", {}).get("model"),
        },
        ...
        "extreme_event_metrics": {
            ...
            "pwv": metrics.get("PWV", {}).get("extreme_event_metrics", {}).get("model"),
            "pwv_object": metrics.get("PWV Object", {}).get("extreme_event_metrics", {}).get("model"),
        },
        "intensity_bin_metrics": {
            ...
            "pwv": metrics.get("PWV", {}).get("intensity_bin_metrics", {}).get("model"),
            "pwv_object": metrics.get("PWV Object", {}).get("intensity_bin_metrics", {}).get("model"),
        },
        "neighborhood_event_metrics": {
            ...
            "pwv": metrics.get("PWV", {}).get("neighborhood_event_metrics", {}).get("model"),
            "pwv_object": metrics.get("PWV Object", {}).get("neighborhood_event_metrics", {}).get("model"),
        },
        "fss": {
            "persistence": ref.get("fss", {}).get("persistence"),
            "radar_only": metrics.get("Radar-only", {}).get("fss", {}).get("model"),
            "pwv": metrics.get("PWV", {}).get("fss", {}).get("model"),
            "pwv_object": metrics.get("PWV Object", {}).get("fss", {}).get("model"),
        },
        "neighborhood_score": {
            "persistence": ref.get("neighborhood_score", {}).get("persistence"),
            "radar_only": metrics.get("Radar-only", {}).get("neighborhood_score", {}).get("model"),
            "pwv": metrics.get("PWV", {}).get("neighborhood_score", {}).get("model"),
            "pwv_object": metrics.get("PWV Object", {}).get("neighborhood_score", {}).get("model"),
        },
        "pearson": {
            "persistence": ref.get("pearson", {}).get("persistence"),
            "radar_only": metrics.get("Radar-only", {}).get("pearson", {}).get("model"),
            "pwv": metrics.get("PWV", {}).get("pearson", {}).get("model"),
            "pwv_object": metrics.get("PWV Object", {}).get("pearson", {}).get("model"),
        },
        "cra": {
            "persistence": ref.get("cra", {}).get("persistence"),
            "radar_only": metrics.get("Radar-only", {}).get("cra", {}).get("model"),
            "pwv": metrics.get("PWV", {}).get("cra", {}).get("model"),
            "pwv_object": metrics.get("PWV Object", {}).get("cra", {}).get("model"),
        },
        "object_metrics": {
            "persistence": ref.get("object_metrics", {}).get("persistence"),
            "radar_only": metrics.get("Radar-only", {}).get("object_metrics", {}).get("model"),
            "pwv": metrics.get("PWV", {}).get("object_metrics", {}).get("model"),
            "pwv_object": metrics.get("PWV Object", {}).get("object_metrics", {}).get("model"),
        },
    }
