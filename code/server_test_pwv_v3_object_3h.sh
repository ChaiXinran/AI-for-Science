...
python -u test_pwv_coupled_v2.py \
    ...
    --pwv_root "${PWV_ROOT}" \
    --checkpoint "${RUN_ROOT}/checkpoints/pwv_v3_object_3h_model.ckpt" \
    --output_dir "${RUN_ROOT}/results/pwv_v3_object_3h" \
    --device "${DEVICE}" \
    ...
    2>&1 | tee "${RUN_ROOT}/logs/test_pwv_v3_object_3h.log"
