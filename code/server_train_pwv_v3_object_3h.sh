...
INIT_GENERATOR="${INIT_GENERATOR:-${RUN_ROOT}/checkpoints/pwv_v3_3h_model.ckpt}"
INIT_ARGS=()
...
python -u train_pwv_coupled_v3.py \
    ...
    --pwv_root "${PWV_ROOT}" \
    --save_dir "${RUN_ROOT}/checkpoints/pwv_v3_object_3h" \
    --readme_ckpt "${RUN_ROOT}/checkpoints/pwv_v3_object_3h_model.ckpt" \
    --device "${DEVICE}" \
    ...
    --lambda_object_center "${LAMBDA_OBJECT_CENTER:-0.05}" \
    --lambda_object_mask "${LAMBDA_OBJECT_MASK:-0.03}" \
    ...
    --log_interval 100 \
    "${INIT_ARGS[@]}" \
    2>&1 | tee "${RUN_ROOT}/logs/train_pwv_v3_object_3h.log"
