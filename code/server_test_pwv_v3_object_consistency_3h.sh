...
python -u test_pwv_coupled_v2.py \
    ...
    --pwv_root "${PWV_ROOT}" \
    --checkpoint "${RUN_ROOT}/checkpoints/pwv_v3_object_consistency_3h_model.ckpt" \
    --output_dir "${RUN_ROOT}/results/pwv_v3_object_consistency_3h" \
    --device "${DEVICE}" \
    ...
    --neighborhood_size 5 \
    --object_thresholds "${OBJECT_THRESHOLDS:-16}" \
    --object_min_area "${OBJECT_MIN_AREA:-4}" \
    --object_iou_threshold "${OBJECT_IOU_THRESHOLD:-0.1}" \
    --psd_lead_minutes "60,120,180" \
    --psd_wavelengths "4,8,16,32,64" \
    --grid_km 1 \
    --frame_minutes "${FRAME_MINUTES:-6}" \
    --horizon_bins "0-1,1-2,2-3" \
    2>&1 | tee "${RUN_ROOT}/logs/test_pwv_v3_object_consistency_3h.log"
