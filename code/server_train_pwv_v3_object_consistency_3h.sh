#!/usr/bin/env bash
set -euo pipefail

DATA_ROOT="${DATA_ROOT:-/root/autodl-tmp/datasets/north_china/DATA_2025_S}"
RUN_ROOT="${RUN_ROOT:-/root/autodl-tmp/nowcastnet_runs/north_china_3h_v3_object_consistency}"
BASELINE_OBJECT_RUN_ROOT="${BASELINE_OBJECT_RUN_ROOT:-/root/autodl-tmp/nowcastnet_runs/north_china_3h_v3_object}"
LEGACY_RUN_ROOT="${LEGACY_RUN_ROOT:-/root/autodl-tmp/nowcastnet_runs/north_china_3h_physical}"
DEVICE="${DEVICE:-cuda:0}"
BATCH_SIZE="${BATCH_SIZE:-8}"
EPOCHS="${EPOCHS:-60}"
NUM_WORKERS="${NUM_WORKERS:-8}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/server_env.sh"
RADAR_ROOT="${RADAR_ROOT:-$(resolve_dataset_dir "${DATA_ROOT}" "RADAR_2025_S" "*RADAR*")}"
RAIN_ROOT="${RAIN_ROOT:-$(try_resolve_dataset_dir "${DATA_ROOT}" "RAIN_2025_S" "*RAIN*" || true)}"
PRECIP_ROOT="${PRECIP_ROOT:-${RAIN_ROOT:-${RADAR_ROOT}}}"
PWV_ROOT="${PWV_ROOT:-$(resolve_dataset_dir "${DATA_ROOT}" "PWV_2025_S" "*PWV*")}"
PRECIP_SCALE="${PRECIP_SCALE:-35}"
PWV_SCALE="${PWV_SCALE:-80}"
FORECAST_LOSS="${FORECAST_LOSS:-weighted_l1}"
FACL_ALPHA="${FACL_ALPHA:-0.1}"
FACL_REDUCTION="${FACL_REDUCTION:-official}"

DEFAULT_INIT="${BASELINE_OBJECT_RUN_ROOT}/checkpoints/pwv_v3_object_3h_model.ckpt"
if [[ ! -f "${DEFAULT_INIT}" ]]; then
    DEFAULT_INIT="${LEGACY_RUN_ROOT}/checkpoints/pwv_v3_object_3h_model.ckpt"
fi
if [[ ! -f "${DEFAULT_INIT}" ]]; then
    DEFAULT_INIT="${LEGACY_RUN_ROOT}/checkpoints/pwv_v3_3h_model.ckpt"
fi
INIT_GENERATOR="${INIT_GENERATOR:-${DEFAULT_INIT}}"
INIT_ARGS=()
if [[ -f "${INIT_GENERATOR}" ]]; then
    INIT_ARGS+=(--init_generator "${INIT_GENERATOR}")
fi

mkdir -p "${RUN_ROOT}/logs" "${RUN_ROOT}/checkpoints"
print_dataset_dir "RADAR_ROOT" "${RADAR_ROOT}"
print_dataset_dir "PRECIP_ROOT" "${PRECIP_ROOT}"
print_dataset_dir "PWV_ROOT" "${PWV_ROOT}"
echo "RUN_ROOT=${RUN_ROOT}"
echo "INIT_GENERATOR=${INIT_GENERATOR}"

python -u train_pwv_coupled_v3.py \
    --data_root "${PRECIP_ROOT}" \
    --pwv_root "${PWV_ROOT}" \
    --save_dir "${RUN_ROOT}/checkpoints/pwv_v3_object_consistency_3h" \
    --readme_ckpt "${RUN_ROOT}/checkpoints/pwv_v3_object_consistency_3h_model.ckpt" \
    --device "${DEVICE}" \
    --model_name PWVCoupledNowcastNetV3Object \
    --input_length 9 \
    --total_length 39 \
    --img_height 96 \
    --img_width 96 \
    --batch_size "${BATCH_SIZE}" \
    --epochs "${EPOCHS}" \
    --num_workers "${NUM_WORKERS}" \
    --stride 1 \
    --train_ratio 0.8 \
    --val_ratio 0.1 \
    --max_train_samples 0 \
    --max_val_samples 0 \
    --intensity_scale "${PRECIP_SCALE}" \
    --pixel_min 0 \
    --pixel_max 255 \
    --forecast_loss "${FORECAST_LOSS}" \
    --facl_alpha "${FACL_ALPHA}" \
    --facl_reduction "${FACL_REDUCTION}" \
    --pwv_intensity_scale "${PWV_SCALE}" \
    --pwv_pixel_min 0 \
    --pwv_pixel_max 255 \
    --pwv_invert \
    --frame_minutes "${FRAME_MINUTES:-6}" \
    --pwv_tendency_windows "${PWV_TENDENCY_WINDOWS:-}" \
    --pwv_tendency_mode "${PWV_TENDENCY_MODE:-slope}" \
    --disc_channels 32 \
    --evo_base_channels 32 \
    --pwv_base_channels 24 \
    --fusion_channels 32 \
    --lead_time_embed_dim "${LEAD_TIME_EMBED_DIM:-16}" \
    --object_head_base_channels "${OBJECT_HEAD_BASE_CHANNELS:-24}" \
    --object_loss_threshold "${OBJECT_LOSS_THRESHOLD:-16}" \
    --object_loss_min_area "${OBJECT_LOSS_MIN_AREA:-4}" \
    --object_center_sigma "${OBJECT_CENTER_SIGMA:-2.0}" \
    --object_consistency_temperature "${OBJECT_CONSISTENCY_TEMPERATURE:-2.0}" \
    --lambda_forecast 1.0 \
    --lambda_evolution 0.5 \
    --lambda_advected 0.25 \
    --lambda_motion 0.02 \
    --lambda_pool 0.2 \
    --lambda_adv 0.01 \
    --lambda_coupling_smooth 0.02 \
    --lambda_coupling_l1 0.001 \
    --lambda_align 0.03 \
    --lambda_shuffle 0.03 \
    --shuffle_margin 0.02 \
    --lambda_false_alarm 0.25 \
    --lambda_support_dry 0.05 \
    --lambda_support_l1 0.01 \
    --lambda_object_center "${LAMBDA_OBJECT_CENTER:-0.04}" \
    --lambda_object_mask "${LAMBDA_OBJECT_MASK:-0.02}" \
    --lambda_object_area "${LAMBDA_OBJECT_AREA:-0.02}" \
    --lambda_object_intensity "${LAMBDA_OBJECT_INTENSITY:-0.02}" \
    --lambda_object_dice "${LAMBDA_OBJECT_DICE:-0.04}" \
    --lambda_object_count "${LAMBDA_OBJECT_COUNT:-0.01}" \
    --lambda_object_centroid "${LAMBDA_OBJECT_CENTROID:-0.01}" \
    --lambda_object_consistency "${LAMBDA_OBJECT_CONSISTENCY:-0.03}" \
    --false_alarm_threshold 2.0 \
    --grad_clip 1.0 \
    --log_interval 100 \
    "${INIT_ARGS[@]}" \
    2>&1 | tee "${RUN_ROOT}/logs/train_pwv_v3_object_consistency_3h.log"
